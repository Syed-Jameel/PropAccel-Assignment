...
export default function Navbar() {
  return (
    <Disclosure as="nav" className="bg-transparent h-[10vh]">
      {({ open }) => (
        <>
            ...
            <div className="relative flex h-16 items-center justify-between">
              ...
              <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-between">
                <div className="flex flex-shrink-0 items-center">
                  <a href="#" className="not-italic font-bold text-base leading-5 text-white">
                    Logo
                  </a>
                </div>
                <div className="hidden sm:ml-6 sm:block">
                  <div className="flex space-x-16">
                    {navigation.map((item) => (
                      <a key={item.name} href={item.href} className="w-12 h-5 hover:text-[#4776E6] not-italic font-normal text-base leading-5 text-white flex-none order-none flex-grow-0">
                        {item.name}
                      </a>
                    ))}
                  </div>
